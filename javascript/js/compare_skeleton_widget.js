...
console.log("[MotionCapture Compare] Loading compare skeleton widget...");

const extensionFolder = detectExtensionFolder();
if (!extensionFolder) {
    console.error("[MotionCapture Compare] Could not detect extension folder");
}

console.log("[MotionCapture Compare] Detected extension folder:", extensionFolder);

app.registerExtension({
    name: "motioncapture.compareskeletons",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "CompareSkeletons") {
            console.log("[MotionCapture Compare] Registering Compare Skeletons node");

            const onNodeCreated = nodeType.prototype.onNodeCreated;
            nodeType.prototype.onNodeCreated = function() {
                const r = onNodeCreated ? onNodeCreated.apply(this, arguments) : undefined;

                console.log("[MotionCapture Compare] Node created, adding compare viewer widget");
                ...
                console.log("[MotionCapture Compare] Setting iframe src to:", viewerUrl);

                iframe.onload = () => {
                    console.log("[MotionCapture Compare] Iframe loaded successfully");
                };
                iframe.onerror = (e) => {
                    console.error("[MotionCapture Compare] Iframe failed to load:", e);
                };
                ...
                console.log("[MotionCapture Compare] Widget created:", widget);
                ...
                if (event.source !== iframe.contentWindow) return;
                    if (event.data && event.data.type === 'VIEWER_READY') {
                        console.log("[MotionCapture Compare] Viewer iframe is ready!");
                ...
                this.onExecuted = function(message) {
                    console.log("[MotionCapture Compare] onExecuted called with message:", message);
                    ...
                    if (leftFile || rightFile) {
                        console.log("[MotionCapture Compare] Loading FBX files - Left:", leftFile, "Right:", rightFile);
                        ...
                        const sendMessage = () => {
                            if (iframe.contentWindow) {
                                console.log("[MotionCapture Compare] Sending postMessage to iframe");
                                iframe.contentWindow.postMessage({
                                ...
                                console.error("[MotionCapture Compare] Iframe contentWindow not available");
                            }
                        };
                        ...
                        if (this.compareViewerReady) {
                            ...
                            setTimeout(() => {
                                clearInterval(checkReady);
                                if (!this.compareViewerReady) {
                                    console.warn("[MotionCapture Compare] Iframe not ready after 2s, sending anyway");
                                    sendMessage();
                                }
                            }, 2000);
                        }
                    } else {
                        console.log("[MotionCapture Compare] No FBX files in message data. Keys:", Object.keys(message || {}));
                    }
                };
...
console.log("[MotionCapture Compare] Compare skeleton widget registered");
