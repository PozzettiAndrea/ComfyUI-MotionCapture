...
class DPVO:
    ...
    def load_long_term_loop_closure(self):
        try:
            from .loop_closure import LongTermLoopClosure
    ...
    def load_weights(self, network):
        ...
        if isinstance(network, str):
            ...
            state_dict = comfy.utils.load_torch_file(str(network))
    ...
    def __call__(self, tstamp, image, intrinsics):
        ...
        if self.n > 0 and not self.is_initialized:
            if self.motion_probe() < 2.0:
                _Id = SE3.Identity(1, device=self.device)
                self.pg.delta[self.counter - 1] = (self.counter - 2, _Id[0])
                return
