...
from .loop_closure import reduce_edges
from .utils import *
...
class PatchGraph:
    ...
    def __init__(self, cfg, P, DIM, pmem, **kwargs):
        ...
        self.kk_inac = torch.as_tensor([], dtype=torch.long, device=self.device)
        self.weight_inac = torch.zeros(1, 0, 2, dtype=torch.long, device=self.device)
        self.target_inac = torch.zeros(1, 0, 2, dtype=torch.long, device=self.device)
    ...
    def edges_loop(self):
        ...
        jj, kk = flatmeshgrid(
            torch.arange(self.n - self.cfg.GLOBAL_OPT_FREQ, self.n - self.cfg.KEYFRAME_INDEX, device=self.device),
            torch.arange(max(l - lc_range, 0) * self.M, l * self.M, device=self.device), indexing='ij')
        ii = self.ix[kk]
